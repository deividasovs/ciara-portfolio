        ...
        <div className={`option1-container fade-in ${isDark ? 'opt1-dark-theme' : ''} ${isCoral ? 'opt1-coral-theme' : ''}`} style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <main className="opt1-section" style={{ flex: 1, width: '100%' }}>
                ...
                {project.images && project.images.length > 0 && (() => {
                    ...
                    const rows: { layout: string, maxHeight?: string, images: { url: string, credit?: string }[] }[] = [];
                    let currentRowGroup: { url: string, credit?: string }[] = [];
                    ...
                    project.images.forEach((item: any) => {
                        if (typeof item === 'string') {
                            ...
                            if (item.url !== topImageInfo.url) {
                                currentRowGroup.push(item as { url: string, credit?: string });
                            }
                            ...
                            if (rowImages.length > 0) {
                                rows.push({ layout: item.layout, images: rowImages });
                            }
                        }
                    });
                    commitCurrentGroup();
                    ...
                    return rows.map((row, rowIndex) => {
                        ...
                        if (row.layout === '1-col') gridStyle.gridTemplateColumns = '1fr';
                            ...
                            if (row.maxHeight) {
                                gridStyle.gridTemplateRows = `repeat(2, minmax(0, calc((${row.maxHeight} - 2rem) / 2)))`;
                            } else {
                                gridStyle.gridTemplateRows = 'repeat(2, minmax(0, 1fr))';
                            }
                        }
                        ...
                        // Apply a maximum track size for images not in the 2-left-1-right manual row configuration
                        if (row.maxHeight && row.layout !== '2-left-1-right') {
                            gridStyle.gridAutoRows = `minmax(0, ${row.maxHeight})`;
                        }
                        ...
                        return (
                            <div key={rowIndex} style={gridStyle}>
                                {row.images.map((img, index) => {
                                    ...
                                    const isHeightConstrained = row.layout === '2-left-1-right' || !!row.maxHeight;

                                    return (
                                        <div key={index} style={{ ...itemStyle, height: row.maxHeight ? '100%' : 'auto' }}>
                                            <div style={{ flex: 1, minHeight: 0, display: 'flex' }}>
                                                <img src={img.url} alt={`${project.title} detail ${index + 1}`} style={{
                                                    width: '100%',
                                                    height: isHeightConstrained ? '100%' : 'auto',
                                                    maxHeight: row.maxHeight ? row.maxHeight : undefined,
                                                    display: 'block',
